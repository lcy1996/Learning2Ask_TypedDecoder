...
class Seq2SeqModel(object):
    ...
    def step_decoder(self, session, data, forward_only=False):
        input_feed = {self.posts: data['posts'],
                ...
                self.keyword_tensor: data['keyword_tensor'],      
                self.word_type: data['word_type']
                }
        if forward_only:
            output_feed = [self.decoder_loss, self.log_perplexity]
        else:
            output_feed = [self.decoder_loss, self.gradient_norm, self.update, self.log_perplexity]
